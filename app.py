...
trending_products = train_data_clean 


app.secret_key = "500124743" 
...
def content_based_recommendations(data_df, item_name, top_n=10,
    ...
    """
    Generates content-based product recommendations from a given DataFrame.
    It uses TF-IDF to vectorize product descriptions (specified by tags_col)
    and then calculates cosine similarity to find items similar to the given 'item_name'.

    Args:
        data_df (pd.DataFrame): The DataFrame containing product data.
        item_name (str): The exact name of the product for which to find recommendations.
        top_n (int): The number of top similar items to recommend.
        product_name_col (str): The name of the column in data_df that contains product names.
        tags_col (str): The name of the column in data_df that contains text for TF-IDF vectorization.
        image_url_col (str): The name of the column in data_df that contains product image URLs.
        brand_col (str): The name of the column in data_df that contains product brand names.
        price_col (str, optional): The name of the column containing product prices. Defaults to None.

    Returns:
        pd.DataFrame: A DataFrame containing details of the recommended items, including
                      product name, brand, image URL, and price (if price_col is provided).
                      Returns an empty DataFrame if the item_name is not found or required columns are missing.
    """
...
def index():
    ...
    Displays a selection of trending products with their actual images and prices from clean_data.csv,
    with fallbacks for missing data.
    """
    # Number of trending products to display
    num_trending_products = 20

    # Initialize lists to hold the image URLs for display
    display_image_urls = []
    display_prices = [] # To store prices

    # Iterate through the first 'num_trending_products' from the trending_products DataFrame
    for index, product in trending_products.head(num_trending_products).iterrows():
        # Get the ImageURL from the product data. Use .get() for safety.
        image_url = product.get('ImageURL')
        price = product.get('Price', 'N/A') # Get price, default to 'N/A' if not found

        # Check if ImageURL is invalid (NaN, not a string, or empty after stripping whitespace)
        if pd.isna(image_url) or not isinstance(image_url, str) or not image_url.strip():
            # If invalid, use a random fallback image
            display_image_urls.append(random.choice(random_image_urls))
        else:
            # Otherwise, use the product's actual ImageURL
            display_image_urls.append(image_url)

        display_prices.append(price) # Add price to the list

    # Column names for product display from clean_data.csv
    ...
    return render_template('index.html',
                           ...
                           trending_product_prices=display_prices, # Pass the list of prices
                           trending_display_name_col=trending_display_name_col,
                           trending_display_image_url_col=trending_display_image_url_col,
                           trending_display_price_col=trending_display_price_col # Pass price column name
                           )

@app.route('/main')
def main():
    """
    Renders the main recommendation page (main.html).
    Initially, it loads with an empty set of content-based recommendations,
    ready for user input in the search bar.
    """
    content_based_rec = pd.DataFrame()  # Initialize an empty DataFrame for recommendations on initial load.
...
def logout():
    """
    Handles user logout.
    Clears the Flask session and redirects the user to the homepage.
    """
...
def indexredirect():
    """
    Redirects from '/index' URL to the root URL ('/') to ensure consistent homepage access.
    This can prevent issues if users manually type '/index'.
    """
...
def signup():
    ...
    On POST request, it retrieves username, email, and password from the form.
    It checks for existing users, hashes the password, and saves new user data to the database.
    Provides feedback (success/failure message) on the homepage.
    """
...
def signin():
    ...
    On POST request, it retrieves username and password from the form.
    It authenticates the user by checking credentials against the database.
    If successful, it sets session variables for the user and redirects to homepage with a message.
    """
...
def recommendations():
    ...
    On POST request, it takes a product name, desired number of recommendations,
    and the chosen dataset from the form.
    """
    if request.method == 'POST':
...
def create_app(test_config=None):
    ...
    if test_config:
        ...
        app.config['SQLALCHEMY_DATABASE_URI'] = 'mysql+pymysql://username:password@localhost/your_db'
        app.config['SQLALCHEMY_TRACK_MODIFICATIONS'] = False
